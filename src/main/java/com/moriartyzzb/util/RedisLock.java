...
public class RedisLock {
    ...
    public RedisLock(RedisTemplate redisTemplate) {
        ...
        //初始化boundValueOps,解决项目启动后，首次执行卡顿过久
        redisTemplate.boundValueOps("redisTemplate").setIfAbsent(10L);
    }
    ...
    public Boolean lock(String key) throws InterruptedException {
        ...
        while (timeOut > 0) {
            ...
            //获取锁失败，查看是否锁已经超时
            Object oTime = redisTemplate.boundValueOps(key).get();
            //可能当前的锁已经过期了或者被释放了,判空
            if(oTime != null){
                long oldExpireTime = (long) oTime;
                //锁已经超时
    ...
    public void unlock(String key) {
        ...
        Object oTime = redisTemplate.boundValueOps(key).get();
        if(oTime != null){
            long expireTimeMillis = (long) oTime;
            //获取当时设置到redis的过期时间
